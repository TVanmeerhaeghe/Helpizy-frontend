<template>
    <div class="article-details">
        ...
        <div class="article-content" v-else>
            <p class="article-date" v-if="lastModifiedDate">Mis à jour le {{ lastModifiedDate }}</p>
            <h1>{{ articleTitle }}</h1>
        ...
        <div v-if="author" class="author-info">
            <div class="author">
                <img :src="author.profile_picture
                    ...
                    : require('../../public/profile.jpg')" alt="Photo de l'auteur" @error="handleAuthorImageError" />
                <p><span class="bold">{{ author.firstname }}</span> <br> {{ author.email }}</p>
            </div>
            <div class="counters">
                <p><img src="../assets/article/eye.svg" alt="Pico Vue">{{ views }}</p>
                <p @click="incrementLike" class="like"><img src="../assets/article/hand-thumbs-up.svg"
                        alt="Picto like">{{
                            article.nb_like }}</p>
                <p><img src="../assets/article/dl.svg" alt="picto téléchargement">5</p>
            </div>
        </div>
    </div>
</template>
    ...
    methods: {
        async fetchArticle(id) {
            try {
                ...
                if (data.id_category) {
                    try {
                        const r2 = await fetch(`${process.env.VUE_APP_API_URL}/category/${data.id_category}`)
                ...
                if (data.id_author) {
                    try {
                        const r3 = await fetch(`${process.env.VUE_APP_API_URL}/author/${data.id_author}`)
        ...
        async incrementLike() {
            ...
            try {
                await fetch(`${process.env.VUE_APP_API_URL}/post/${this.article.id_post}`, {
                    method: 'PATCH',
