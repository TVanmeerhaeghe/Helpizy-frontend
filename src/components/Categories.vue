<template>
  <section class="categories">
    <div class="info-cat">
      <h4 class="title-36">
        Explore nos <span class="title-blue">tutoriels</span> pratiques
      </h4>
      <p class="font-20">
        Notre plateforme vous propose une multitude de guides détaillés et d'informations claires pour
        vous simplifier la vie sur différents thèmes :
      </p>
    </div>

    <div class="all-cat">
      <div class="block-cat" v-for="cat in categories" :key="cat.id" @click="goToCategory(cat.id)">
        <img :src="cat.imgSrc" alt="" />
        <p class="title-cat">{{ cat.title }}</p>
        <div class="separator"></div>
        <p class="txt-cat">{{ cat.description }}</p>
      </div>
    </div>
  </section>
</template>
    ...
    return {
      categories: [
        ...
        },
        {
          ...
          description:
            "Prime d’activité, aide au transport, aides locales pour le logement social, chèque énergie,.."
        }
      ]
    }
  },
